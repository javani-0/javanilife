...
// ---------------------------------------------------------------------------
// Assignments for ONE class (req 2): set work, then review the PDFs students
// submit. Class-scoped so it works from the Academics page and from the
// Academics tabs inside Classes Manager alike.
// ---------------------------------------------------------------------------
...
const AdminAssignmentsPanel = ({ selectedClass }: { selectedClass: ClassDoc | undefined }) => {
  ...
  const create = async () => {
    ...
    try {
      ...
      toast({ title: "Assignment created", description: `Students in ${selectedClass.name} can see it now.` });
  ...
  if (!classId) {
    return (
      <p className="rounded-xl border border-dashed border-border p-10 text-center font-body text-sm text-muted-foreground">
        Choose a class to set and review its assignments.
      </p>
    );
  }
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-gold/25 bg-gold/5 p-4">
        <p className="flex items-center gap-1.5 font-body text-sm font-semibold text-foreground">
          <Plus className="h-4 w-4 text-gold" /> New assignment
        </p>
...
export default AdminAssignmentsPanel;
