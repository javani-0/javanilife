...
import { Check, Loader2, Save, Users, X } from "lucide-react";
...
// ---------------------------------------------------------------------------
// Attendance roster for ONE class (req P2, req 1). Class-scoped so it can be
// used both from the standalone Attendance page and from the Academics tabs
// inside Classes Manager, where the class is already chosen.
// ---------------------------------------------------------------------------
...
const AdminAttendancePanel = ({ selectedClass }: { selectedClass: ClassDoc | undefined }) => {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-border/60 bg-card p-4 shadow-card">
        ...
        <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className={`${inputClass} sm:max-w-[220px]`} />
      </div>
