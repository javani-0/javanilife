...
- **TYPECHECK GOTCHA:** the root `tsconfig.json` has `"files": []` and only project references, so **`npx tsc --noEmit -p tsconfig.json` checks NOTHING and always reports success.** Always typecheck with **`npx tsc --noEmit -p tsconfig.app.json`** (client) and `npx tsc --noEmit -p api/tsconfig.json` (server). Every "typecheck" step in this plan means those two commands.
- **Pre-existing type errors** (present on a clean tree — ignore, do not fix here): 5 in `src/components/admin/CategoryManager.tsx` (TS2352) and 2 in `src/lib/ecommerce/categories.ts` (TS2677).
